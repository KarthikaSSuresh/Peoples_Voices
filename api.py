...
def userviewfeedback():
	...
	q="SELECT * FROM `feedbacks` WHERE `sender_id`='%s' ORDER BY `feedback_id` DESC"%(log_id)
	print(q)
	result = select(q)
	if result:
		data['status'] = 'success'
...
def user_view_missing_persons():
	...
	q="SELECT *,CONCAT(first_name,' ',last_name) as pname FROM `missing_persons` WHERE `status` !='found' ORDER BY `missing_person_id` DESC"
	print(q)
	result = select(q)
	if result:
		data['status'] = 'success'
...
def user_view_wanted_criminals():
	...
	q="SELECT *,concat(first_name,' ',last_name) as cname FROM `criminals` INNER JOIN `crime_categories` USING(`category_id`) WHERE `most_wanted`='most_wanted' ORDER BY `criminal_id` DESC"
	result = select(q)
	if result:
		data['status'] = 'success'
		data['data'] = result
...
def user_send_found_report():
	...
	if ftype=="mperson":
		...
		print(q)
		id=insert(q)
	elif ftype=="mcriminal":
		q= "INSERT INTO `criminal_alert` VALUES(NULL,(select user_id from users where login_id='%s'),'%s','0','%s',curdate(),'pending')"%(loginid,mp_ids,placename)
		id=insert(q)
...
def user_view_found_report():
	...
	q="SELECT * FROM `missing_found` WHERE `missing_person_id`='%s' ORDER BY `found_id` DESC"%(mp_ids)
	result = select(q)
	if result:
		data['status'] = 'success'
		data['data'] = result
...
def user_view_complaints():
	...
	q="SELECT * FROM `complaints` INNER JOIN `files` USING(`complaint_id`) WHERE `user_id`=(SELECT `user_id` FROM `users` WHERE `login_id`='%s') ORDER BY `complaint_id` DESC"%(login_id)
	result = select(q)
	if result:
		data['status'] = 'success'
		data['data'] = result
