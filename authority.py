...
def authority_view_criminal_alert():
	...
	# q="SELECT *,vehicle_number AS username,CONCAT(`criminals`.`first_name`,' ',`criminals`.`last_name`) AS `criminal_name`,`criminal_alert`.`status` AS criminal_status FROM `criminal_alert` INNER JOIN `vehicles` on vehicles.vehicle_id=criminal_alert.user_id INNER JOIN `criminals` USING(criminal_id)"
	print(q)
	res=select(q)
